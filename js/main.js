...
function animateThings(currTime) {
  ...
  if (wordHighlight) {
    ...
    if (progress > totalDuration) {
      ...
      let numIterations = Math.ceil(Math.random() * (maxNumIterations - minNumIterations)) + minNumIterations;
    ...
    let progress3 = currTime - lastChangedTime;
...
// Little movement on the Letter
const RContainer = document.querySelector(".R-container"),
  reduction = 0.005;
document.body.onmousemove = function (e) {
  if (document.body.scrollTop < window.innerHeight && RContainer != null) {
    let horizScale = -(e.clientX / window.innerWidth - 0.5) * 100 * reduction;
    let vertScale = -(e.clientY / window.innerHeight - 0.5) * 100 * reduction * 2;

    RContainer.style.opacity = 1;
    RContainer.style.animation = "none";

    RContainer.style.transform = "translateX(" + horizScale + "%) translateY(" + vertScale + "%)";
  }
}
